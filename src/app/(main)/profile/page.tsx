"use client";

import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { motion } from "framer-motion";
import { Camera } from "lucide-react";

export default function ProfilePage() {
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      {/* Profile Section */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      >
        <Card className="border-none bg-white rounded-2xl shadow-none overflow-hidden">
          <CardContent className="p-8 space-y-8">
            {/* Photo Upload Section */}
            <div className="flex items-center gap-6">
              <div className="relative group">
                <Avatar className="h-24 w-24 rounded-full border-0">
                  <AvatarImage
                    src="https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&q=80&w=200&h=200"
                    alt="Profile"
                    className="object-cover"
                  />
                  <AvatarFallback className="bg-primary/10 text-primary text-xl font-bold">SJ</AvatarFallback>
                </Avatar>
                <button className="absolute bottom-0 right-0 p-1.5 bg-white rounded-full border border-gray-100 shadow-sm cursor-pointer hover:scale-105 transition-transform text-[#2ECC71]">
                  <div className="bg-[#E6F9F1] p-1 rounded-full">
                    <Camera size={14} />
                  </div>
                </button>
              </div>
              <div className="flex flex-col gap-1">
                <h3 className="text-lg font-bold text-gray-800">Profile Photo</h3>
                <p className="text-sm text-gray-400 font-medium">JPG, GIF or PNG. Max size of 800K</p>
                <div className="flex items-center gap-4 mt-1">
                  <button className="text-sm font-bold text-primary hover:underline cursor-pointer">Update Image</button>
                  <button className="text-sm font-bold text-destructive hover:underline cursor-pointer">Remove</button>
                </div>
              </div>
            </div>

            {/* Form Fields */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              <div className="space-y-2.5">
                <Label className="text-[15px] font-bold text-gray-700">Full Name</Label>
                <Input
                  defaultValue="Sarah Jenkins"
                  className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-700 font-medium"
                />
              </div>
              <div className="space-y-2.5">
                <Label className="text-[15px] font-bold text-gray-700">Email Address</Label>
                <Input
                  defaultValue="s.jenkins@carefy.portal"
                  className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-700 font-medium"
                />
              </div>
              <div className="space-y-2.5">
                <Label className="text-[15px] font-bold text-gray-700">Role</Label>
                <Input
                  defaultValue="Admin"
                  className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-700 font-medium"
                />
              </div>
              <div className="space-y-2.5">
                <Label className="text-[15px] font-bold text-gray-700">Employee ID</Label>
                <Input
                  defaultValue="CF-1092"
                  className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-700 font-medium"
                />
              </div>
            </div>
          </CardContent>
        </Card>
      </motion.div>

      {/* Change Password Section */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.1 }}
      >
        <Card className="border-none bg-white rounded-2xl shadow-none overflow-hidden">
          <CardContent className="p-8 space-y-8">
            <h2 className="text-xl font-bold text-gray-800 border-b border-gray-50 pb-6">Change Password</h2>

            <div className="space-y-6">
              <div className="space-y-2.5">
                <Label className="text-[15px] font-bold text-gray-700">Current Password</Label>
                <Input
                  type="password"
                  placeholder="Enter your current password here..."
                  className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-500 font-medium"
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="space-y-2.5">
                  <Label className="text-[15px] font-bold text-gray-700">New Password</Label>
                  <Input
                    type="password"
                    placeholder="Enter your new password here..."
                    className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-500 font-medium"
                  />
                </div>
                <div className="space-y-2.5">
                  <Label className="text-[15px] font-bold text-gray-700">Confirm Password</Label>
                  <Input
                    type="password"
                    placeholder="Enter your confirm password here..."
                    className="h-14 bg-[#F5F6FF]/50 border-none rounded-xl px-6 focus-visible:ring-primary shadow-none text-gray-500 font-medium"
                  />
                </div>
              </div>
            </div>

            <div className="pt-4 flex justify-end">
              <button className="bg-primary text-white font-bold py-4 px-10 cursor-pointer rounded-xl hover:bg-primary/90 transition-all">
                Save Changes
              </button>
            </div>
          </CardContent>
        </Card>
      </motion.div>
    </div>
  );
}
